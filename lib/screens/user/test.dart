  ...
  Widget build(BuildContext context) {
    ...
    if (Platform.isAndroid) {
      contentList.addAll([
        ...
        ListTile(
          title: Text("Aspect tolerance (${_aspectTolerance.toStringAsFixed(2)})"),
          subtitle: Slider(
            min: -1.0,
            max: 1.0,
    ...
    contentList.addAll(_possibleFormats.map(
      (format) => CheckboxListTile(
        ...
        onChanged: (i) {
          setState(() => selectedFormats.contains(format) ? selectedFormats.remove(format) : selectedFormats.add(format));
        },
